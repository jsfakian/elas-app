...
func Insert(db *sql.DB, ap, violationNumber, decisionDate string) {
	log.Println("Inserting decision record ...")
	insertDecisionSQL := `INSERT INTO decisions(ap, violation_number, decision_date) 
		VALUES (?, ?, ?)`
	statement, err := db.Prepare(insertDecisionSQL) // Prepare statement. This is good to avoid SQL injections
	if err != nil {
		log.Error(err)
	}
	_, err = statement.Exec(ap, violationNumber, decisionDate)
	if err != nil {
		log.Error(err)
	}
}
...
func basicGet(db *sql.DB, query string, arg ...interface{}) []*Decision {
	...
	for row.Next() { // Iterate and fetch the records from result cursor
		...
		err = row.Scan(&decision.AP, &decision.ViolationNumber, &decision.DecisionDate, &decision.PublishDate)
		if err != nil {
			log.Error(err)
		}
		log.Info(decision)
...
func GetByAll(db *sql.DB) []*Decision {
	return basicGet(db, `SELECT ap, violation_number, decision_date, publish_date FROM decisions`)
}

func GetByViolationNumber(db *sql.DB, violationNumber string) []*Decision {
	return basicGet(db, `SELECT ap, violation_number, decision_date, publish_date FROM decisions WHERE 
	violation_number like ? `, violationNumber)
}

func GetByDecisionDate(db *sql.DB, decisionDate string) []*Decision {
	return basicGet(db, `SELECT ap, violation_number, decision_date, publish_date FROM decisions WHERE decision_date 
	like ?`, decisionDate)
}
